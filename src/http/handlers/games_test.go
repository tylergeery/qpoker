...
func TestCreateGameSuccess(t *testing.T) {
	...
	assert.Equal(t, 5, game.Options["time_between_hands"])
	assert.Equal(t, int64(10)*bigBlind, game.Options["buy_in_max"])
	assert.Equal(t, bigBlind, game.Options["buy_in_min"])
...
func TestGameUpdateSuccess(t *testing.T) {
	...
	assert.Equal(t, int64(60), updatedGame.Options["big_blind"])
